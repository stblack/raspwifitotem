...
step=$((step+1))
echo -e "\n************************************************************"
echo "Backup dei file di configurazione attuali e scarico la configurazione da github online"
...
if validate_url https://raw.githubusercontent.com/stblack/raspwifitotem/master/etc/dhcpcd.conf; then
...
if validate_url https://raw.githubusercontent.com/stblack/raspwifitotem/master/etc/dnsmasq.conf; then
    mv /etc/dnsmasq.conf /etc/dnsmasq.conf.orig
    wget -P /etc/ https://raw.githubusercontent.com/stblack/raspwifitotem/master/etc/dnsmasq.conf
fi

if validate_url https://raw.githubusercontent.com/stblack/raspwifitotem/master/etc/network/interfaces; then
    mv /etc/network/interfaces /etc/network/interfaces.orig
    wget -P /etc/network https://raw.githubusercontent.com/stblack/raspwifitotem/master/etc/network/interfaces
fi

if validate_url https://raw.githubusercontent.com/stblack/raspwifitotem/master/etc/lighttpd/lighttpd.conf; then
    mv /etc/lighttpd/lighttpd.conf /etc/lighttpd/lighttpd.conf.orig
    wget -P /etc/lighttpd https://raw.githubusercontent.com/stblack/raspwifitotem/master/etc/lighttpd/lighttpd.conf
fi

if validate_url https://raw.githubusercontent.com/stblack/raspwifitotem/master/etc/hostapd/hostapd.conf; then
...
echo "Step $step : scaricamento file configurazione eseguita"
step=$((step+1))
echo -e "\n************************************************************"
echo "Patch pagina per bypassare la ricerca del server google in background dentro android 6 "
...
echo "Step $step : applicazione patch android 6 eseguita"
...
echo "Step $step : set demone hostapd eseguito"
...
echo "Step $step : Fine Configurazione"
echo -e "************************************************************"